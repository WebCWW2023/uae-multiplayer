...
const addBanner = () => {
  ...
  fetch(`${globalUrl}usertexter?room_name=${roomName}`, requestOptions)
...
textureUpdateForm.addEventListener("submit", (e) => {
  ...
  if (bannerToUpload && bannerToUpload.type === "image/jpeg") {
    if (bannerToUpload.size > 1000000) {
      ...
      picture.addEventListener("load", function (event) {
        ...
        fetch(`${globalUrl}updateaddusertexter`, requestOptions)
          .then((response) => response.json())
          .then((result) => {
            UpdateTexture(result.data.text_image, selectedBannername);
          })
          .then(() => {
